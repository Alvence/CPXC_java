...
public class GradientBasedSampler implements Sampler {
	...
	public GradientBasedSampler(double delta, double eps, double gamma){
		this.delta = delta;
		this.eps = eps;
		this.gamma = gamma;
	}
...
*/
	...
	public Instances samplingFromInstance(AbstractClassifier cl, Instances headerInfo, Instance instance, int N) {
		Instances ret = new Instances(headerInfo, 0);
		int count = 0;
		Instance center = (Instance)instance.copy();
		double[] sumC = new double[instance.numAttributes()-1];
		for(int i =0;i < instance.numAttributes()-1;i++){
			sumC[i] = instance.value(i);
		}
		while(count<N){
			Instance newIns = (Instance)instance.copy();
			Instance cur = null;
			double[] dir = new double[instance.numAttributes()-1];
			for(int i =0;i < instance.numAttributes()-1;i++){
				dir[i] = instance.value(i) - sumC[i]/count;
			}
			while(cur==null || distance(cur,newIns) > 1e-6){
			try {
				cur = (Instance)newIns.copy();
				List<Double> gr = calcGradient(cl, headerInfo, cur);
				//normalize the gradient
