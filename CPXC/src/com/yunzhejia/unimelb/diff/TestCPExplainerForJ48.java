...
import com.yunzhejia.unimelb.diff.TestCPExplainer.CPStrategy;
import com.yunzhejia.unimelb.diff.TestCPExplainer.FPStrategy;
import com.yunzhejia.unimelb.diff.TestCPExplainer.PatternSortingStrategy;
import com.yunzhejia.unimelb.diff.TestCPExplainer.SamplingStrategy;
...
public class TestCPExplainerForJ48 {
...
//		RandomExplainer app = new RandomExplainer();
		try {
			...
			for(String file:files){
				PrintWriter writerExpl = new PrintWriter(new File("tmp/"+file+"_expl.txt"));
				PrintWriter writerSup = new PrintWriter(new File("tmp/"+file+"_sup.txt"));
				PrintWriter writerRatio = new PrintWriter(new File("tmp/"+file+"_ratio.txt"));
				PrintWriter writerNum = new PrintWriter(new File("tmp/"+file+"_num.txt"));
				PrintWriter writerDim = new PrintWriter(new File("tmp/"+file+"_dim.txt"));
...
//			for(int ratio:ratios)
			...
			TestCPExplainer app = new TestCPExplainer();
//			RandomExplainer app = new RandomExplainer();
...
//			System.out.println(goldFeatures);
			...
			for(Instance ins:test){
				...
				List<IPattern> expls = app.getExplanations(FPStrategy.APRIORI, samplingStrategy, 
						...
						cl, ins, train, numOfSamples, 0.15, 3, numOfExpl, false, stat);
				writerSup.println(stat.sup);
				writerRatio.println(stat.ratio);
				writerNum.println(stat.numOfPatterns);
				writerDim.println(stat.instrinsicDimension);
...
//					System.out.println(expls.size()+"  precision="+precision);
					...
					writerExpl.println(ExplEvaluation.evalF1Best(expls, goldFeatures));
				}else{
//					System.err.println("No explanations!"+ c);
					writerExpl.println(0);
				}
				}catch(Exception e){
					throw e;
//					e.printStackTrace();
